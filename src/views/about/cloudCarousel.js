...
(function ($) {
	...
	function Reflection(img, reflHeight, opacity) {
		...
		this.element = reflection = parent
			.append("<canvas class='reflection' style='position:absolute'/>")
			.find(":last")[0];
		if (!reflection.getContext && $.browser.msie) {
			this.element = reflection = parent
				.append("<img class='reflection' style='position:absolute'/>")
				.find(":last")[0];
	...
	var Item = function (imgIn, options) {
		...
		if (this.options.reflHeight > 0) {
			this.reflection = new Reflection(
				this.image,
				this.options.reflHeight,
				this.options.reflOpacity
			);
		}
	...
	var Controller = function (container, images, options) {
		...
		this.innerWrapper = $(container)
			.wrapInner('<div style="position:absolute;width:100%;height:100%;"/>')
			.children()[0];
		...
		this.updateAll = function () {
			...
			for (var i = 0; i < itemsLen; i++) {
				...
				x =
					this.xCentre +
					(funcCos(radians) * this.xRadius - item.orgWidth * 0.5) * scale;
		...
		this.checkImagesLoaded = function () {
			...
			for (i = 0; i < images.length; i++) {
				if (
					images[i].width === undefined ||
					(images[i].complete !== undefined && !images[i].complete)
				) {
					return;
				}
			}
	...
	$.fn.CloudCarousel = function (options) {
		this.each(function () {
			...
			$(this).data(
				"cloudcarousel",
				new Controller(this, $(".cloudcarousel", $(this)), options)
			);
		});
		return this;
	};
